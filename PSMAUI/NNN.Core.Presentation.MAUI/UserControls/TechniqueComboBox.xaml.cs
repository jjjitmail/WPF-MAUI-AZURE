...
public partial class TechniqueComboBox : ContentView
{
    ...
    public static readonly BindableProperty psItemsSourceProperty = BindableProperty.Create(nameof(psItemsSource),
        typeof(ObservableCollection<MethodGroup>), typeof(TechniqueComboBox), default(ObservableCollection<MethodGroup>), BindingMode.TwoWay);
    ...
    public static readonly BindableProperty psSelectedItemProperty = BindableProperty.Create(nameof(psSelectedItem),
        typeof(Models.Method), typeof(TechniqueComboBox), default(Models.Method), BindingMode.TwoWay);

    public static readonly ControlTemplate psComboBoxTemplate = new ControlTemplate(typeof(PSComboBoxTemplate));

    public ObservableCollection<MethodGroup> psItemsSource
    {
        get => (ObservableCollection<MethodGroup>)GetValue(psItemsSourceProperty);
        set { SetValue(psItemsSourceProperty, value); OnPropertyChanged("psItemsSource"); }
    }

    public Models.Method psSelectedItem
    {
        get
        {
            return (Models.Method)GetValue(psSelectedItemProperty);
        }
        set
        {
    ...
    protected override void OnPropertyChanged(string propertyName = null)
    {
        ...
        {
            //CollectionViewRoot.SetBinding(ItemsView.ItemsSourceProperty, "TechniqueItemSources");

            CollectionViewRoot.ItemsSource = psItemsSource;
        }
        else if (propertyName == psSelectedItemProperty.PropertyName)
        {
            SelectedLblName1.Text = psSelectedItem.MethodName;
            SelectedLblIcon1.Points = psSelectedItem.MethodIcon;
            SelectedLblIcon1.Stroke = Color.FromArgb(PSColor.DefaultBlueColor);
            SelectedLblName2.Text = psSelectedItem.MethodName;
            SelectedLblIcon2.Points = psSelectedItem.MethodIcon;
    ...
    private void CollectionView_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        PSTasks.ActionTask(() =>
        {
            ...
            var current = e.CurrentSelection.FirstOrDefault() as Models.Method;
